 ...
 * 
 */
export class AutomationConnectionTagsApi extends runtime.BaseAPI {
    ...
    async updateConnectionTagsRaw(requestParameters: UpdateConnectionTagsRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<runtime.ApiResponse<void>> {
        ...
        const response = await this.request({
            path: `/connections/{id}/connection-tags`.replace(`{${"id"}}`, encodeURIComponent(String(requestParameters.id))),
            method: 'PUT',
            headers: headerParameters,
            query: queryParameters,
