import {useReportQueryError} from '@/shared/error/useReportQueryError';
import {
    AiAutoMemoriesQuery,
...
export const AiAutoMemoriesKeys = {
    ...
    // The environment and the principal pair are part of the detail key because a memory id alone no longer
    // identifies a row the caller can reach: the same id read in another environment or as another owner resolves to
    // null rather than to this row.
    detail: (
        memoryId: number,
        workspaceId: number,
        environmentId: number,
...
export function useAiAutoMemoryQuery(
    ...
    const query = useGeneratedAiAutoMemoryQuery<AiAutoMemoryI | null, Error>(
        ...
        },
        {
            ...
            queryKey: AiAutoMemoriesKeys.detail(memoryId ?? -1, workspaceId, environmentId, principalType, principalId),
...
export function useUpdateAiAutoMemoryMutation() {
    ...
    return useGeneratedUpdateAiAutoMemoryMutation({
        onSuccess: (_data, variables) => {
            ...
            // Prefix of the detail key without the principal pair, so the cached detail is refreshed for every owner.
            queryClient.invalidateQueries({
                queryKey: [
                    ...AiAutoMemoriesKeys.all,
                    'detail',
