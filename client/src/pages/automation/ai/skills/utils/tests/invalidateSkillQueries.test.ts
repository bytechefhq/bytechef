...
describe('invalidateSkillQueries', () => {
    it('invalidates the skill list, the tag facet and all skill detail queries', () => {
        const queryClient = new QueryClient();

        queryClient.setQueryData(['aiSkills'], {aiSkills: [{id: '1'}]});
        queryClient.setQueryData(['aiSkillTags'], {aiSkillTags: [{id: '1', name: 'Billing'}]});
        ...
        expect(queryClient.getQueryState(['aiSkillTags'])?.isInvalidated).toBe(true);
