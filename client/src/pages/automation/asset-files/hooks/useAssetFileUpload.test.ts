...
describe('useAssetFileUpload', () => {
    ...
    test('upload posts multipart with workspaceId and file', async () => {
        const {result} = renderHook(() => useAssetFileUpload());
        const file = new File(['# hi'], 'spec.md', {type: 'text/markdown'});

        const response = await result.current.upload(42, 0, file);
    ...
    test('throws on non-ok response', async () => {
        ...
        await expect(result.current.upload(42, 0, file)).rejects.toThrow();
    });
});
