...
> saveWorkflowTestConfigurationConnection(workflowId, workflowNodeName, workflowConnectionKey, environmentId, saveWorkflowTestConfigurationConnectionRequest)
...
import type { SaveWorkflowTestConfigurationConnectionOperationRequest } from '';
...
async function example() {
  ...
  const body = {
    ...
    saveWorkflowTestConfigurationConnectionRequest: ...,
  } satisfies SaveWorkflowTestConfigurationConnectionOperationRequest;

  try {
    const data = await api.saveWorkflowTestConfigurationConnection(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
