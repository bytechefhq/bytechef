...
vi.mock('@/shared/middleware/graphql', () => ({
    useMyWorkspaceScopesQuery: () => hoisted.queryResult,
}));
...
describe('useLoadWorkspaceScopes', () => {
    ...
    it('marks the workspace as loading while the query is in flight', () => {
        ...
        expect(permissionStore.getState().workspaceScopeStates[WORKSPACE_ID]).toEqual({status: 'loading'});
    });
    ...
    it('stores fetched scopes when the query succeeds', () => {
        ...
        expect(permissionStore.getState().workspaceScopeStates[WORKSPACE_ID]).toEqual({
    ...
    it('marks the workspace as errored, fires a toast, and clears stale scopes on fetch failure', () => {
        // Critical fail-closed path: keeping the stale cache on error would let useHasWorkspaceScope return true for
        ...
        permissionStore.setState({
            workspaceScopeStates: {[WORKSPACE_ID]: {scopes: ['WORKFLOW_VIEW'], status: 'loaded'}},
            workspaceStates: {},
        });
        ...
        expect(permissionStore.getState().workspaceScopeStates[WORKSPACE_ID]).toEqual({status: 'error'});
