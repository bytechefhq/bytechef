...
import {createAiHubTask, generateAiHubTaskTitle} from './tasks.api';
...
describe('tasks.api', () => {
    ...
    it('createAiHubTask sends GraphQL variables matching the createAiHubTask mutation', async () => {
        // The schema declares createAiHubTask(workspaceId: ID!, environment: Int!, threadId: String!) — pin
        ...
        const result = await createAiHubTask({environment: 0, threadId: 'thread-x', workspaceId: 42});
        ...
        expect(body.query).toContain('createAiHubTask');
    ...
    it('generateAiHubTaskTitle calls the generateAiHubTaskTitle mutation with id+workspaceId', async () => {
        const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue(
            new Response(
                JSON.stringify({
                    data: {
        ...
        const result = await generateAiHubTaskTitle({taskId: 7, workspaceId: 42});

        const body = JSON.parse(fetchSpy.mock.calls[0]![1]?.body as string);

        expect(body.query).toContain('generateAiHubTaskTitle');
        expect(body.variables).toEqual({
            id: '7',
            workspaceId: '42',
        });
    ...
    it('generateAiHubTaskTitle throws with the server message when the upstream model is unavailable', async () => {
        ...
        await expect(generateAiHubTaskTitle({taskId: 7, workspaceId: 42})).rejects.toThrow(
