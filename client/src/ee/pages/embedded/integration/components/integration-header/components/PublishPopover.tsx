...
const PublishPopover = ({
    isPending,
    onPublishIntegrationSubmit,
}: {
    isPending: boolean;
    onPublishIntegrationSubmit: ({description, onSuccess}: {description?: string; onSuccess: () => void}) => void;
}) => {
    ...
    const handlePublishIntegration = ({description}: {description?: string}) => {
        onPublishIntegrationSubmit({
            description,
            onSuccess: () => {
                reset();
    ...
    return (
        <Popover onOpenChange={setOpen} open={open}>
            ...
            >
                <Form {...form}>
                    <form className="flex flex-col gap-4" onSubmit={handleSubmit(handlePublishIntegration)}>
