...
const ApiConnectorManualPage = () => {
    const {currentStep, endpoints, icon, name, nextStep, previousStep, reset, setCurrentStep, specification} =
    ...
    const stepCount = WIZARD_STEPS.manual.length;
    ...
    const getPageTitle = () => {
        const stepName = WIZARD_STEPS.manual[currentStep] || '';

        return `Create API Connector - ${stepName}`;
    };
                    ...
                    <div className="flex flex-col gap-1 border-b p-6">
                        ...
                        <nav aria-label="Progress">
                            <ol className="space-y-4 md:flex md:space-y-0" role="list">
                                {WIZARD_STEPS.manual.map((stepLabel, index) => (
