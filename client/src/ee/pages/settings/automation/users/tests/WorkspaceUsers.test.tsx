...
const hoisted = vi.hoisted(() => ({
    ...
    scopesByEnvironment: {} as Record<string, string[]>,
...
vi.mock('@/shared/middleware/graphql', () => ({
    ...
    useMyWorkspaceScopesQuery: vi.fn((variables: {environment?: string}) => ({
        data: {myWorkspaceScopes: hoisted.scopesByEnvironment[variables.environment ?? ''] ?? hoisted.scopes},
        isLoading: false,
    })),
    ...
    useRemoveWorkspaceUserMutation: vi.fn((options: {onSuccess: () => void}) => {
        ...
        return {mutate: hoisted.removeWorkspaceUserMutate};
    }),
...
describe('WorkspaceUsers', () => {
    ...
    it('renders for a workspace admin who is not a tenant admin', () => {
        // The page is reachable on the workspace scope rather than ROLE_ADMIN, unlike every sibling settings route.
    ...
    it('withholds the add-existing-user tab from a stale admin account with no session', async () => {
        // The store clears `authenticated` before `getAccount()` reconciles `account` for the incoming user, so during a
        // logout/re-login transition the previous session's ROLE_ADMIN is still on `account`. Reading the authority
        // alone would hand the new user the admin-only tenant user picker for that window.
    ...
    it('locks an inherited row', () => {
        ...
        // One remove button, for the real member only — an inherited entry has no row to remove. It names the member it
        // acts on, so a screen reader can tell two near-identical rows apart.
    ...
    it('removes a stored member once confirmed', async () => {
        render(<WorkspaceUsers />);

        await userEvent.click(screen.getByRole('button', {name: 'Remove member@example.com from workspace'}));

        expect(screen.getByText('Remove member@example.com from the workspace?')).toBeInTheDocument();
        expect(hoisted.removeWorkspaceUserMutate).not.toHaveBeenCalled();
    ...
    it('keeps a member when removal is cancelled', async () => {
        ...
        expect(hoisted.removeWorkspaceUserMutate).not.toHaveBeenCalled();
        expect(screen.queryByText('Remove member@example.com from the workspace?')).not.toBeInTheDocument();
    });

    it("invalidates the caller's own permission queries after a membership mutation", () => {
        // A membership mutation can move the operator's own access -- removing themselves, demoting themselves, or
        // trading a built-in role for a custom one. Invalidating only ['WorkspaceUsers'] left myWorkspaceScopes cached,
        // so this page went on offering Invite and Remove against an answer the server no longer gives.
        render(<WorkspaceUsers />);
    ...
    it('explains that removing the only environment role keeps that role in every environment', async () => {
        ...
        expect(screen.getByText(/keep the .* role in every environment/i)).toBeInTheDocument();
    });

    it('warns about that environment when the member holds others', async () => {
        hoisted.workspaceUsers = [developmentRole, productionRole];
    ...
    it('asks for the member-manage scope in every environment', () => {
        render(<WorkspaceUsers />);

        const environments = vi
            .mocked(useMyWorkspaceScopesQuery)
            .mock.calls.map(([variables]) => (variables as {environment?: string}).environment);

        expect(new Set(environments)).toEqual(new Set(['DEVELOPMENT', 'PRODUCTION', 'STAGING']));
    });

    it('withholds workspace-wide member controls from a member who manages members in only some environments', () => {
        hoisted.scopesByEnvironment = {PRODUCTION: ['WORKSPACE_VIEW'], STAGING: ['WORKSPACE_VIEW']};
        ...
        expect(screen.queryByRole('button', {name: 'Invite User'})).not.toBeInTheDocument();
        expect(
            screen.queryByRole('button', {name: 'Remove member@example.com from workspace'})
        ).not.toBeInTheDocument();
    });

    it('lets a member change environment roles only where they manage members', async () => {
        hoisted.scopesByEnvironment = {PRODUCTION: ['WORKSPACE_VIEW'], STAGING: ['WORKSPACE_VIEW']};
        hoisted.workspaceUsers = [developmentRole, productionRole];

        render(<WorkspaceUsers />);

        expect(await screen.findByRole('combobox', {name: /development role/i})).toBeEnabled();
        expect(screen.getByRole('combobox', {name: /production role/i})).toBeDisabled();
        expect(screen.getByRole('button', {name: /remove development role/i})).toBeInTheDocument();
        expect(screen.queryByRole('button', {name: /remove production role/i})).not.toBeInTheDocument();
        expect(screen.queryByRole('combobox', {name: 'Grant a role in another environment'})).not.toBeInTheDocument();
    });
