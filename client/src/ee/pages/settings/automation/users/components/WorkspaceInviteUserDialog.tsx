...
interface WorkspaceInviteUserDialogPropsI {
     ...
     * through invite-by-email instead, which reuses the existing account rather than provisioning a second one.
     */
...
const WorkspaceInviteUserDialog = ({
    ...
    // Radix renders the trigger as a button rather than a form control, so a visible `<label htmlFor>` cannot reach
    // it. The label carries the accessible name through aria-labelledby instead, and each tab passes its own label id
    // so the two role pickers are not both called by the same element.
    const renderRoleSelect = (labelId: string) => (
            ...
            <DialogContent>
                ...
                <Tabs onValueChange={setTab} value={tab}>
                    ...
                    <TabsContent className="space-y-4 pt-4" value={INVITE_TAB}>
                        ...
                        <div className="space-y-2">
                            <label className="text-sm font-medium" id={inviteRoleLabelId}>
                                Role
                            </label>

                            {renderRoleSelect(inviteRoleLabelId)}
                        </div>
                    </TabsContent>
                    ...
                    {isTenantAdmin && (
                        <TabsContent className="space-y-4 pt-4" value={ADD_TAB}>
                            <p className="text-xs text-muted-foreground">
                                Adds someone who already has a ByteChef account. No email is sent.
                            </p>

                            <div className="space-y-2">
                                <label className="text-sm font-medium" id={userLabelId}>
                                    User
                                </label>

                                <Select onValueChange={setAddUserId} value={addUserId}>
                                    <SelectTrigger aria-labelledby={userLabelId}>
                            ...
                            <div className="space-y-2">
                                <label className="text-sm font-medium" id={addRoleLabelId}>
                                    Role
                                </label>

                                {renderRoleSelect(addRoleLabelId)}
                            </div>
                        </TabsContent>
                    )}
                </Tabs>
