...
1. Run the client successfully: [Setup with Docker](#setup-with-docker). Code is located in `BYTECHEF_HOME/client`.
2. Add your changes.
...
1. Run the server successfully: [Local Setup](#local-setup). Code is located in `BYTECHEF_HOME/server`. Components are located at `BYTECHEF_HOME/server/libs/modules/components`.
2. Add your changes.
...
1. Code is located in `BYTECHEF_HOME/docs`. To run it, you need to run the commands below from the folder:
    ```bash
   ./npm install
   ./npm run dev
    ```
...
#### Initial login credentials available:
