---
title: "Telegram"
description: "Telegram is a cloud-based messaging platform that enables users to send messages, media, and files, and supports automation and integrations through its API."
---
...
| webhookSecretToken | Webhook Secret Token | STRING | Optional secret token set when registering this bot's webhook (setWebhook) at &lt;public-url&gt;/telegram/interactivity. When set, field-less approval requests use in-place Approve/Discard buttons resolved through that webhook, verified against this token. Use a bot dedicated to approvals, since a bot has a single webhook. Leave empty to deliver approval links to the hosted form. | false |
