...
| returnValue | Return Value | STRING <details> <summary> Options </summary> <span title="Get the attribute value like 'class' from an element.">ATTRIBUTE</span>, <span title="Get the HTML content that the element contains.">HTML</span>, <span title="Get the text content of the element.">TEXT</span> </details> | Specifies what content should be extracted from the matched element(s). | true |
