...
description: Build your first browser voice workflow with a Deepgram voice agent, test it in the editor, and wire it to the chat widget or AI Hub.
ee: true
comingSoon: true
---
...
This is the recommended v1 setup. Splitting the chain across providers (e.g. Deepgram STT + Anthropic LLM + ElevenLabs TTS) is possible, but stick with all-in-one for your first voice workflow.
...
See the [widget README](https://github.com/bytechefhq/bytechef/blob/master/sdks/frontend/automation/chat/library/README.md) for the full config reference.
...
6. Save
7. Open any AI Hub chat and click the mic button

How it works: Deepgram does STT and TTS only; AI Hub's routing agent runs the LLM in between. Your workspace's configured LLM provider (set in admin → AI gateway settings), task overlay, tools, and chat memory continuity all apply to voice turns the same way they apply to text turns. The session is bound to the AI Hub chat's threadId so voice and text turns share the same session memory — ask the same chat something in voice mode, then switch to text, and the LLM remembers the voice turn.
...
A 5-minute voice call = ~$0.65. Production deployments should set per-workspace concurrent-session limits to prevent runaway costs.
...
- [Testing voice workflows in the editor](./editor-testing) — iterate on prompts and models without deploying
