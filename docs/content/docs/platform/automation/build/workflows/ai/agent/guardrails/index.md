...
ByteChef ships **12 guardrail components** — 10 child detectors plus the two parent actions they compose into — [Check For Violations](./check-for-violations) (block on violation) and [Sanitize Text](./sanitize-text) (mask in place). You wire them together in the workflow editor like any other action.
...
You wire both parents as cluster elements in the AI Agent's **advanced editor** (the Guardrails slot is not a section of the simple editor's configuration panel): attach **Check For Violations** and/or **Sanitize Text** in the Guardrails slot, attach a shared **Model** child if any LLM-stage detector is used, then attach the child detectors inside each parent.
...
The 10 child guardrails split into two execution stages:
