...
Effective agentic systems are usually built from a small set of composable patterns rather than one monolithic "do everything" agent. ByteChef lets you build each of these patterns on the workflow canvas by combining [flow controls](/platform/automation/build/workflows/flow-controls) with the [AI Agent](/platform/automation/build/workflows/ai/agent) — no framework code required.
...
Decompose a task into a fixed sequence of steps, where each step's output feeds the next. In ByteChef this is simply a **linear sequence of tasks** on the canvas — one or more AI Agent (or LLM) steps in order, passing data with [data pills](/platform/automation/build/workflows/data-pills). Add a [Condition](/reference/flow-controls/condition_v1) between steps as a gate when a stage should only run if the previous output passes a check.
