...
<Callout type="warn">
Write the key as `#this['email']` rather than a bare `email`. An item without the key then gives
`null` in the result. With the bare form, the whole formula can come back unevaluated when the
first item in the list has no such key.
</Callout>
