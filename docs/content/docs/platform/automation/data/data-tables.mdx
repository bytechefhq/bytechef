...
![The Data Table grid - typed columns, two rows selected with the row checkboxes, and a number cell open in its editor](data-tables/data-table-grid.png)

![The grid's actions menu, with Import CSV, Export CSV, Rename Table and Delete Table](data-tables/data-table-actions-menu.png)
...
[Data Tables API reference](/openapi/automation-data-tables-data-table) for tables and columns, and
the [Data Table Rows reference](/openapi/automation-data-tables-data-table-row) for rows. Rows can be
queried with `filter=column:OPERATOR:value`, written in batches, imported and exported as CSV, and
upserted on a caller-supplied `externalId`.
