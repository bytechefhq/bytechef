...
Before diving into creating workflows, it's helpful to understand some key concepts. Check them in the [Glossary](/platform/glossary).

## Creating Your First Workflows

Follow these steps to create your first workflows in ByteChef Automation:

<Callout title="Generate with AI">
  You can also create a workflow from a description: in a project's workflow list, choose **Create Workflow → Generate with AI** to open the [AI Copilot](/platform/automation/build/ai-copilot#generating-a-workflow-or-skill-from-a-prompt) panel in Build mode, pre-scoped to building that workflow. Requires Copilot to be enabled.
</Callout>

<Callout title="Quick Start Guides">
  For step-by-step tutorials with specific examples, check out our Quick Start guides:
  - [Star a Repository on GitHub](/platform/automation/get-started/quick-start/star-repository-on-github)
  - [Configure workflow trigger](/platform/automation/get-started/quick-start/configure-workflow-trigger)
</Callout>
