...
The database file lives at `~/.bytechef/bytechef.mv.db`, next to the encryption key and the
generated remember-me key, so all three survive recreating the container. Deleting that file discards every workflow, connection, and execution
...
extension and stay unavailable on H2. Persistent AI chat memory is also PostgreSQL-only, so the H2
profile switches chat memory to `inmemory` for you - AI agents work, but conversations are forgotten
