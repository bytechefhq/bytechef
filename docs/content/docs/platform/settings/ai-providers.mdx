...
> Activating providers is restricted to **admins**.

> A provider can also be configured without the UI, through configuration properties / environment variables
> such as `BYTECHEF_AI_PROVIDER_OPEN_AI_API_KEY`. See [Self-hosting: static keys](#self-hosting-static-keys).
...
<Callout title="Models are configuration">
    The model itself is always configuration - the AI Providers page stores credentials and
    enablement, never model names. The API key can be configuration too
    (`BYTECHEF_AI_PROVIDER_OPEN_AI_API_KEY` and friends), so a deployment can set the embedding model
    and the key it authenticates with entirely by property.
</Callout>
...
**Precedence:** a provider activated through the UI for an environment always wins. The static configuration key is used only as a fallback when no provider is activated for that environment. This lets an operator ship a default key while still letting each environment override it through the UI.
