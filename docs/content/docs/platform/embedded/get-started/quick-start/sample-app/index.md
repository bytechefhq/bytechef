...
| **App Event** (`/app-event`) | Firing an **App Event** from your application to start every one of the connected user's enabled App Event-triggered workflows. | [`POST /api/embedded/v1/app-events`](/openapi/embedded-webhook-app-event-trigger) |
| **Request** (`/request`) | Triggering a single workflow synchronously via its **Request trigger** and reading the workflow's response back. | [`POST /api/embedded/v1/workflows/{workflowUuid}`](/openapi/embedded-webhook-request-trigger) |
