...
Before diving into creating workflows, it's helpful to understand some key concepts. Check them in the [Glossary](/glossary).
...
<Callout title="Generate with AI">
  You can also create a workflow from a single sentence: in a project's workflow list, choose **Create Workflow → Generate with AI**, describe what you want, and the [AI Copilot](/platform/copilot#one-shot-generation-from-a-prompt) builds the whole workflow for you. Requires Copilot to be enabled.
</Callout>
