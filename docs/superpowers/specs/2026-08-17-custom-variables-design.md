...
| Duplicate name in scope+environment | `VARIABLE_NAME_ALREADY_EXISTS`; for workspace-scoped rows guarded by `uk_property_key_scope_scope_id_environment` if two admins race — the second commit fails, surfaced as the same error. Embedded rows (and any other `Scope`/null-`scope_id` row that sets an environment, e.g. `AiProviderFacadeImpl`'s platform-scoped properties) have a NULL `scope_id`, which Postgres treats as distinct per row, so that constraint does not fire there; the partial unique index `uk_property_key_scope_environment_null_scope_id` (`20260825000001`) closes that gap, and `VariableServiceImpl` catches the resulting `DataIntegrityViolationException`/`DuplicateKeyException` from either constraint and translates it to the same error |
