...
- **New:** `server/ee/libs/automation/automation-ai/automation-ai-gateway/automation-ai-gateway-otlp/`
  (api + service submodules, sibling to existing gateway submodules). OTLP Protobuf generation via the
  `build.buf.build/opentelemetry/opentelemetry-proto` artifact; no hand-rolled schemas.
...
- **New:** `server/ee/libs/automation/automation-ai/automation-ai-gateway/automation-ai-gateway-dataset/`
  and `.../automation-ai-gateway-experiment/` (api + service + rest submodules each). These sit alongside
  the existing `automation-ai-gateway-{api,service,public-rest}` siblings rather than as platform-level
  modules — datasets + experiments are EE-only gateway concerns, not general platform infrastructure.
