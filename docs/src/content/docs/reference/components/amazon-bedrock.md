...
| Return Likelihoods | STRING | SELECT  |  The token likelihoods are returned with the response.  |
| Truncate | STRING | SELECT  |  Specifies how the API handles inputs longer than the maximum token length  |
