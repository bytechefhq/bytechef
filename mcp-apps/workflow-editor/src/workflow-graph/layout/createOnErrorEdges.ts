...
export function getOnErrorBranchSide(
    ...
    const inMainBranch = Array.isArray(mainBranch)
        ? mainBranch.some((task: WorkflowTaskType) => task.name === onErrorChildTaskId)
        : false;
 ...
 */
export function hasTaskInOnErrorBranches(onErrorId: string, taskId: string, tasks: WorkflowTaskType[]): boolean {
    ...
    return allBranchTasks.some((task: WorkflowTaskType) => task.name === taskId);
}
...
export default function createOnErrorEdges(onErrorNode: Node, allNodes: Node[]): Edge[] {
    ...
    const mainBranchSubtasks: WorkflowTaskType[] = Array.isArray(parameters?.[ON_ERROR_WIRE_KEY_MAIN_BRANCH])
        ? parameters[ON_ERROR_WIRE_KEY_MAIN_BRANCH]
        : [];
    const errorBranchSubtasks: WorkflowTaskType[] = Array.isArray(parameters?.[ON_ERROR_WIRE_KEY_ERROR_BRANCH])
