...
// The read-only layout pipeline never authors workflows, so the SOURCE's
// getParametersWithDefaultValues (which reads component property definitions) is not
// available. getInitialParameters is authoring-only and unused by layout; this is a
// no-op stand-in that preserves the SOURCE call shape.
// eslint-disable-next-line @typescript-eslint/no-unused-vars
function getParametersWithDefaultValues(_props: {properties: Array<PropertyAllType>}): Record<string, unknown> {
    return {};
}

function branchCaseKeysMatch(left: string | number, right: string | number): boolean {
...
export const TASK_DISPATCHER_CONFIG = {
    branch: {
        ...
        updateTaskParameters: ({context, task, updatedSubtasks}: UpdateTaskParametersType): WorkflowTaskType => {
            const caseKey = context?.caseKey || 'default';
    ...
    each: {
        ...
        getSubtasks: ({node, task}: {node?: Node; task?: WorkflowTaskType}): Array<WorkflowTaskType> => {
            const parameters = (node?.data as NodeDataType)?.parameters || task?.parameters;
        ...
        updateTaskParameters: ({task, updatedSubtasks}: UpdateTaskParametersType): WorkflowTaskType => ({
            ...task,
            parameters: {
                ...task.parameters,
    ...
    loopBreak: {
        ...
        getTask: getTaskDispatcherTask,
        initializeParameters: () => ({}),
        updateTaskParameters: ({task}: UpdateTaskParametersType): WorkflowTaskType => task,
    },
    map: {
        buildNodeData: ({baseNodeData, taskDispatcherContext, taskDispatcherId}: BuildNodeDataType): NodeDataType =>
    ...
    'on-error': {
        ...
        getSubtasks: ({
            ...
            const mainBranch = parameters[ON_ERROR_WIRE_KEY_MAIN_BRANCH] as WorkflowTaskType[] | undefined;
            const errorBranch = parameters[ON_ERROR_WIRE_KEY_ERROR_BRANCH] as WorkflowTaskType[] | undefined;
 ...
 */
export function getParentTaskDispatcherTask(taskId: string, tasks: WorkflowTaskType[]): WorkflowTaskType | undefined {
...
export function getTaskDispatcherTask({
    taskDispatcherId,
    tasks,
}: GetParentTaskDispatcherTaskType): WorkflowTaskType | undefined {
