 ...
 */
function createLoopSubtaskEdges(loopId: string, loopChildTasks: Array<WorkflowTaskType>): Edge[] {
    const edges: Edge[] = [];
...
export default function createLoopEdges(loopNode: Node): Edge[] {
    ...
    if (!nodeData.parameters?.iteratee?.length) {
        ...
        const loopChildTasks: Array<WorkflowTaskType> = nodeData.parameters.iteratee;
