 ...
 */
function createMapSubtaskEdges(mapId: string, mapChildTasks: Array<WorkflowTaskType>): Edge[] {
    const edges: Edge[] = [];
...
export default function createMapEdges(mapNode: Node): Edge[] {
    ...
    if (!nodeData.parameters?.iteratee?.length) {
        ...
        const mapChildTasks: Array<WorkflowTaskType> = nodeData.parameters.iteratee;
