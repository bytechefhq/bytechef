...
export default function createForkJoinEdges(forkJoinNode: Node): Edge[] {
    ...
    const branchCount =
        nodeData.parameters?.branches?.filter((branch: WorkflowTaskType[]) => branch.length > 0).length ?? 0;
    ...
    const branches: WorkflowTaskType[][] = Array.isArray(nodeData.parameters?.branches)
        ? nodeData.parameters.branches.map((branch: WorkflowTaskType[] | WorkflowTaskType) =>
              Array.isArray(branch) ? branch : branch ? [branch] : []
          )
        : [];
    ...
    if (!hasSubtasks) {
        ...
        leftBranches.forEach((branch: WorkflowTaskType[]) => {
        ...
        rightBranches.forEach((branch: WorkflowTaskType[]) => {
