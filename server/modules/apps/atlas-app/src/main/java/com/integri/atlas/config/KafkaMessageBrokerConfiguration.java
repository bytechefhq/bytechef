...
import com.integri.atlas.engine.message.broker.Exchanges;
import com.integri.atlas.engine.message.broker.Queues;
import com.integri.atlas.engine.message.broker.kafka.KafkaMessageBroker;
...
public class KafkaMessageBrokerConfiguration implements KafkaListenerConfigurer {
    ...
    public ProducerFactory<String, Object> producerFactory(
        ...
        KafkaProperties aKafkaProperties
    ) {
    ...
    private void registerListenerEndpoint(
        KafkaListenerEndpointRegistrar aRegistrar,
        String aQueueName,
        int aConcurrency,
        Object aDelegate,
        String aMethodName
        ...
        Method listenerMethod = Stream
            .of(aDelegate.getClass().getMethods())
            .filter(it -> aMethodName.equals(it.getName()))
            .findFirst()
            .orElseThrow(() ->
                new IllegalArgumentException("No method found: " + aMethodName + " on " + aDelegate.getClass())
            );

        final MethodKafkaListenerEndpoint<String, String> endpoint = createListenerEndpoint(
            aQueueName,
            aDelegate,
            listenerMethod
        );
