 ...
 * @author Ivica Cardic
 */
public abstract class AbstractTaskDescriptorHandlerResolver implements TaskDescriptorHandlerResolver {

    private final ExtTaskDescriptorHandlerRepository extTaskDescriptorHandlerRepository;
    private final String type;

    protected AbstractTaskDescriptorHandlerResolver(
            ExtTaskDescriptorHandlerRepository extTaskDescriptorHandlerRepository, String type) {
    ...
    public List<TaskDescriptorHandler> getTaskDescriptorHandlers() {
        return extTaskDescriptorHandlerRepository.findAllNamesByType(type).stream()
                .flatMap(nameVersions -> nameVersions.versions().stream()
