 ...
 */
@Component
public class ShutdownApplicationListener implements DisposableBean {
    ...
    public void destroy() throws Exception {
        descriptorExtHandlerService.remove(taskDescriptorHandlerResolver.getTaskDescriptorHandlers().stream()
                .map(TaskDescriptorHandler::getTaskDescriptor)
                .map(TaskDescriptor::getName)
                .toList());
    }
}
