...
public class PostgresJdbcJobRepository implements JobRepository {
  ...
  public Job merge (Job aJob) {
    ...
    jdbc.update("update job set status=:status,start_time=:startTime,end_time=:endTime,current_task=:currentTask,workflow_id=:workflowId,label=:label,outputs=(:outputs)::jsonb where id = :id ", sqlParameterSource);
    return aJob;
  }
  ...
  public void create (Job aJob) {
    MapSqlParameterSource sqlParameterSource = createSqlParameterSource(aJob);
    jdbc.update("insert into job (id,create_time,start_time,status,current_task,workflow_id,label,priority,inputs,webhooks,outputs,parent_task_execution_id) values (:id,:createTime,:startTime,:status,:currentTask,:workflowId,:label,:priority,(:inputs)::jsonb,(:webhooks)::jsonb,(:outputs)::jsonb,:parentTaskExecutionId)", sqlParameterSource);
  }
