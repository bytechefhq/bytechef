...
package com.bytechef.atlas.service.job;
...
public class JobService {
    ...
    public Job create(MapObject jobParams) {
        ...
        job.setId(jobParams.containsKey(Constants.ID) ? jobParams.getString(Constants.ID) : UUIDGenerator.generate());
    ...
    public List<Job> getJobs() {
        ...
        for (Job job : jobs) {
            ((SimpleJob) job)
                    .setExecutions(taskExecutions.stream()
    ...
    public Optional<Job> getLatestJob() {
        return jobRepository.findLatestJob().map(job -> {
            ((SimpleJob) job).setExecutions(taskExecutionRepository.findAllByJobIdOrderByCreateTime(job.getId()));
