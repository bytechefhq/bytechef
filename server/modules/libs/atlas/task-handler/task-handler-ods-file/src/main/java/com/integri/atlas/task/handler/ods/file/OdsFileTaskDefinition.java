 ...
 */
@Component
public class OdsFileTaskDefinition implements TaskDefinition {
    ...
    public static final TaskSpecification TASK_SPECIFICATION = TaskSpecification
        ...
        .properties(
            SELECT_PROPERTY(PROPERTY_OPERATION)
                ...
                .options(
                    option("Read from file", Operation.READ.name(), "Reads data from a ODS file."),
                    option("Write to file", Operation.WRITE.name(), "Writes the data to a ODS file.")
                )
