    ...
    public Object handle(TaskExecution aTask) {
        try {
            ...
            if (connection.getResponseCode() / 100 == 2) {
                ...
                Consumer<Integer> progressConsumer = p -> eventPublisher.publishEvent(
                        WorkflowEvent.of(Events.TASK_PROGRESSED, "taskId", aTask.getId(), "progress", p));

                try (BufferedInputStream in = new BufferedInputStream(connection.getInputStream());
                        OutputStream os = new ProgressingOutputStream(
                                new FileOutputStream(downloadedFile), contentLength, progressConsumer)) {
                    copy(in, os);
                }
