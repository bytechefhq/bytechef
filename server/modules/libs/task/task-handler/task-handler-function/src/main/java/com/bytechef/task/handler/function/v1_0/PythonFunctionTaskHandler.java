 ...
 */
@Component(FUNCTION + "/" + VERSION_1_0 + "/" + PYTHON)
public class PythonFunctionTaskHandler implements TaskHandler<Object> {

    private final PolyglotEngine polyglotEngine;

    public PythonFunctionTaskHandler(PolyglotEngine polyglotEngine) {
        this.polyglotEngine = polyglotEngine;
    }

    @Override
    public Object handle(TaskExecution taskExecution) {
        return polyglotEngine.execute(PYTHON, taskExecution);
    }
}
