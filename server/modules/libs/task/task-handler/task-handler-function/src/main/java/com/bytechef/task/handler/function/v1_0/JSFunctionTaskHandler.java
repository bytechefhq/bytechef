 ...
 */
@Component(FUNCTION + "/" + VERSION_1_0 + "/" + JS)
public class JSFunctionTaskHandler implements TaskHandler<Object> {

    private final PolyglotEngine polyglotEngine;

    public JSFunctionTaskHandler(PolyglotEngine polyglotEngine) {
        this.polyglotEngine = polyglotEngine;
    }

    @Override
    public Object handle(TaskExecution taskExecution) {
        return polyglotEngine.execute(JS, taskExecution);
    }
}
