...
public class RSocketConfiguration {
    ...
    @Bean
    public Flux<List<LoadbalanceTarget>> coordinatorLoadBalancedTargets(DiscoveryClient discoveryClient) {
        return Mono.fromSupplier(() -> discoveryClient.getInstances("coordinator-service-app"))
            .repeatWhen(longFlux -> longFlux.delayElements(Duration.ofSeconds(2)))
            .map(this::toLoadBalanceTarget);
    }

    @Bean
    RSocketRequester coordinatorRSocketRequester(
        DiscoveryClient discoveryClient, RSocketStrategies rSocketStrategies) {
        ...
        return RSocketRequester.builder()
            ...
            .transports(coordinatorLoadBalancedTargets(discoveryClient), new RoundRobinLoadbalanceStrategy());
    }
