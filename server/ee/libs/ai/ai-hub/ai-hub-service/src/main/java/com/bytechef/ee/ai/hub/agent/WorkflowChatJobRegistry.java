...
/**
 * Per-chat jobId registry for in-flight workflow-chat turns. The streaming executor emits a
 ...
 * call. Without the registry the cancel mutation has no way to resolve chatId to jobId — the AG-UI per-turn runId isn't
 * the same as the executor's job identifier, and the chat row itself doesn't track in-flight job ids.
 *
 * <p>
...
public class WorkflowChatJobRegistry {
    ...
    /**
     * Records the in-flight job id for the given chat. Called by {@link AgUiStreamBridge} when the executor emits
     * {@code event=start, payload.jobId=...}. Overwrites any prior entry — a chat with a previous jobId still in the
     * cache (e.g. the previous turn's job that finished but whose entry hasn't yet been evicted) gets its slot replaced
     * by the current turn.
     */
    public void register(long chatId, long jobId) {
        getCache().put(chatId, jobId);
    }

    /**
     * Atomically clears the registered jobId for a chat when the turn finishes. Idempotent — calling with no entry
    ...
    public void clear(long chatId) {
        getCache().evictIfPresent(chatId);
    }

    /**
     * Returns the current jobId for a chat, or {@code null} if none is registered. Called by the cancel-mutation
     ...
     * complexity.
     */
    public @Nullable Long get(long chatId) {
        return getCache().get(chatId, Long.class);
    }
        ...
        if (cache == null) {
            throw new IllegalStateException(
                "WorkflowChatJobRegistry cache '" + CACHE_NAME + "' is not configured. Add it to the "
                    ...
                    + "workflow-chat cancellation can resolve chatId to jobId.");
        }
