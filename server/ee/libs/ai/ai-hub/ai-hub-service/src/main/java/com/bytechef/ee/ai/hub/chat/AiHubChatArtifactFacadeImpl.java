...
/**
 * Implementation of {@link AiHubChatArtifactFacade}. Carries the {@code ADMIN} guard and the workspace access check,
 * and delegates to {@link AiHubChatArtifactService} for the audit listing.
 *
 * @version ee
 *
 * @author Ivica Cardic
...
public class AiHubChatArtifactFacadeImpl implements AiHubChatArtifactFacade {
    ...
    public AiHubChatArtifactFacadeImpl(
        AiHubChatArtifactService chatArtifactService, UserService userService, WorkspaceFacade workspaceFacade) {
    ...
    public AiHubChatArtifactPage getAiHubChatArtifacts(
        long workspaceId, @Nullable Integer environment, @Nullable Long userId, @Nullable AiHubChatArtifactKind kind,
        ...
        List<AiHubChatArtifact> items = chatArtifactService.listByWorkspace(
            workspaceId, environment, userId, kind, fromLdt, toLdt, boundedPage, boundedSize);

        long totalCount = chatArtifactService.countByWorkspace(
            workspaceId, environment, userId, kind, fromLdt, toLdt);

        boolean hasMore = ((long) boundedPage + 1) * boundedSize < totalCount;

        return new AiHubChatArtifactPage(items, totalCount, hasMore, pageClamped, sizeClamped);
    }
}
