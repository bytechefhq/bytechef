...
/**
 * Default implementation of {@link AiHubChatArtifactService}.
 *
 * @version ee
 *
 * @author Ivica Cardic
...
@Transactional
public class AiHubChatArtifactServiceImpl implements AiHubChatArtifactService {

    /**
     * Hard upper bound on the number of artifacts returned by {@link #listByChat}. Without this cap a long-running chat
    ...
    private static final List<Integer> WORKFLOW_KIND_ORDINALS = List.of(
        AiHubChatArtifactKind.WORKFLOW_CREATED.ordinal(),
        AiHubChatArtifactKind.WORKFLOW_UPDATED.ordinal(),
        AiHubChatArtifactKind.WORKFLOW_REFERENCED.ordinal());

    private static final Logger log = LoggerFactory.getLogger(AiHubChatArtifactServiceImpl.class);

    private static final RowMapper<AiHubChatArtifact> ARTIFACT_ROW_MAPPER = (resultSet, rowNum) -> {
        AiHubChatArtifact artifact = new AiHubChatArtifact();

        artifact.setId(resultSet.getLong("id"));
        artifact.setChatId(resultSet.getLong("chat_id"));
        artifact
            .setKind(EnumOrdinals.fromOrdinal(resultSet.getInt("kind"), AiHubChatArtifactKind.class));
        ...
        artifact.setStatus(
            EnumOrdinals.fromOrdinal(resultSet.getInt("status"), AiHubChatArtifactStatus.class));
    ...
    private final AiHubChatArtifactRepository chatArtifactRepository;
    ...
    public AiHubChatArtifactServiceImpl(
        AiHubChatArtifactRepository chatArtifactRepository,
        AiHubChatRepository chatRepository,
        JdbcTemplate jdbcTemplate,
        JsonMapper jsonMapper) {

        this.chatArtifactRepository = chatArtifactRepository;
    ...
    @Override
    public void record(
        String threadId, long userId, AiHubChatArtifactKind kind,
        String artifactId, String artifactName, @Nullable Map<String, Object> metadata) {

        Optional<AiHubChat> chatOptional =
            chatRepository.findByThreadIdAndUserId(threadId, userId);
        ...
        if (chatOptional.isEmpty()) {
            ...
            // logs.
            log.warn(
                "No chat found for threadId={} userId={} kind={} artifactId={} artifactName={} — " +
        ...
        artifact.setChatId(chat.getId());
        artifact.setKind(kind);
        artifact.setArtifactId(artifactId);
        artifact.setArtifactName(artifactName);
        // Denormalise the parent chat's environment onto the artifact row. The audit-listing UI filters by
        // environment per row and the JOIN cost on a multi-million-row table is non-trivial; carrying the value at
        // write time is cheaper than paying the join on every page load. The chat's environment is itself
        // immutable post-create (see AiHubChatServiceImpl.create), so this is a safe one-time copy.
        artifact.setEnvironment(chat.getEnvironment());
        ...
        artifact.setStatus(AiHubChatArtifactStatus.APPLIED);

        chatArtifactRepository.save(artifact);
    }

    @Override
    public void recordWorkflowArtifact(
        String threadId, @Nullable Long userId, AiHubChatArtifactKind kind,
        ...
        Optional<AiHubChat> chatOptional = chatRepository.findByThreadIdAndUserId(threadId, userId);

        if (chatOptional.isEmpty()) {
            log.warn(
                "No chat found for threadId={} userId={} — DROPPING workflow artifact (workflowId={}, name={}).",
        ...
        Optional<AiHubChatArtifact> existingOptional =
            chatArtifactRepository.findFirstByChatIdAndArtifactIdAndKindIn(
                chat.getId(), workflowId, WORKFLOW_KIND_ORDINALS);

        AiHubChatArtifact artifact;
        ...
        if (existingOptional.isPresent()) {
            ...
            artifact = new AiHubChatArtifact();
    ...
    @Override
    public AiHubChatArtifact recordReference(
        ...
        AiHubChat chat = chatRepository.findById(chatId)
            .orElseThrow(() -> new NotFoundException("AiHubChat not found: " + chatId));

        if (chat.getUserId() != requesterUserId) {
            throw new ForbiddenException(
                "User " + requesterUserId + " does not own chat " + chatId);
        }

        if (!Objects.equals(chat.getWorkspaceId(), requesterWorkspaceId)) {
            throw new ForbiddenException(
                "AiHubChat " + chatId + " does not live in workspace " + requesterWorkspaceId);
        }
        ...
        // one row" rule across all workflow kinds; mirror it here by matching on (chatId, artifactId) across
        // the workflow kinds rather than scoping to WORKFLOW_REFERENCED alone. Non-workflow kinds keep the
        // exact (chatId, kind, artifactId) tuple, which matches the artifact-list rendering.
        Optional<AiHubChatArtifact> existing =
            WORKFLOW_KIND_ORDINALS.contains(kind.ordinal())
                ? chatArtifactRepository.findFirstByChatIdAndArtifactIdAndKindIn(
                    chatId, artifactId, WORKFLOW_KIND_ORDINALS)
                : chatArtifactRepository.findFirstByChatIdAndKindAndArtifactId(
                    chatId, kind.ordinal(), artifactId);
        ...
        return chatArtifactRepository.save(artifact);
    }

    @Override
    public @Nullable AiHubChatArtifact recordReferenceByThread(
        String threadId, @Nullable Long userId, AiHubChatArtifactKind kind, String artifactId,
        String artifactName) {
        ...
        Optional<AiHubChatArtifact> existing = chatArtifactRepository.findFirstByChatIdAndKindAndArtifactId(
            chat.getId(), kind.ordinal(), artifactId);
    ...
    public void deleteReference(long artifactId, long requesterWorkspaceId, long requesterUserId) {
        Optional<AiHubChatArtifact> artifactOptional = chatArtifactRepository.findById(artifactId);
        ...
        AiHubChatArtifact artifact = artifactOptional.get();

        AiHubChatArtifactKind kind = artifact.getKind();

        if (kind != AiHubChatArtifactKind.FILE_REFERENCED
            && kind != AiHubChatArtifactKind.WORKFLOW_REFERENCED
            && kind != AiHubChatArtifactKind.DATA_TABLE_REFERENCED
            && kind != AiHubChatArtifactKind.KB_REFERENCED) {
        ...
        AiHubChat chat = chatRepository.findById(artifact.getChatId())
            .orElseThrow(() -> new NotFoundException("AiHubChat not found: " + artifact.getChatId()));

        if (chat.getUserId() != requesterUserId) {
            throw new ForbiddenException("User " + requesterUserId + " does not own chat " + chat.getId());
        }

        if (!Objects.equals(chat.getWorkspaceId(), requesterWorkspaceId)) {
            throw new ForbiddenException(
                "AiHubChat " + chat.getId() + " does not live in workspace " + requesterWorkspaceId);
        }

        chatArtifactRepository.deleteById(artifactId);
    }

    @Override
    @Transactional(readOnly = true)
    public List<AiHubChatArtifact> listByChat(
        long chatId, long requesterWorkspaceId, long requesterUserId) {
        ...
        if (!Objects.equals(chat.getWorkspaceId(), requesterWorkspaceId)) {
            throw new ForbiddenException(
                "AiHubChat " + chatId + " is not in workspace " + requesterWorkspaceId);
        }

        return chatArtifactRepository.findByChatIdOrderByCreatedAtDesc(
            chatId, Limit.of(LIST_LIMIT));
    }

    @Override
    @Transactional(readOnly = true)
    public long countByChat(long chatId) {
        return chatArtifactRepository.countByChatId(chatId);
    }

    @Override
    @Transactional(readOnly = true)
    public List<AiHubChatArtifact> listByWorkspace(
        long workspaceId, @Nullable Integer environment, @Nullable Long userId,
        @Nullable AiHubChatArtifactKind kind, @Nullable LocalDateTime from, @Nullable LocalDateTime to,
        int page, int size) {

        StringBuilder sqlBuilder = new StringBuilder(
            "SELECT a.id, a.chat_id, a.kind, a.artifact_id, a.artifact_name, " +
                "a.metadata_json, a.status, a.environment, a.created_at " +
                "FROM ai_hub_chat_artifact a " +
                "JOIN ai_hub_chat c ON c.id = a.chat_id " +
    ...
    public long countByWorkspace(
        long workspaceId, @Nullable Integer environment, @Nullable Long userId,
        @Nullable AiHubChatArtifactKind kind, @Nullable LocalDateTime from,
    ...
    private void appendWorkspaceFilters(
        ...
        @Nullable Long userId, @Nullable AiHubChatArtifactKind kind,
        ...
        if (environment != null) {
            ...
            // before the chat table is materialised. Combined with the idx_cc_chat_artifact_environment
     ...
     */
    private String
        serializeMetadata(Map<String, Object> metadata, AiHubChatArtifactKind kind, String artifactId) {
        try {
