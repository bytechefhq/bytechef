...
/**
 * Publishes integration audit events through Spring Boot's actuator audit bus. Mirrors {@code AiHubAuditPublisher} —
 ...
 *
 * <p>
 * Failures absorb silently into {@code bytechef_integration_audit_failed} + an error log; emission must never break the
 * just-succeeded business transaction. Unauthenticated callers get the {@code "SYSTEM"} principal fallback.
 *
 * @version ee
 *
 * @author Ivica Cardic
...
public class IntegrationAuditPublisher {

    private static final Logger log = LoggerFactory.getLogger(IntegrationAuditPublisher.class);
    ...
    public IntegrationAuditPublisher(
        ...
        this.auditFailureCounter = meterRegistry == null ? null
            : Counter.builder("bytechef_integration_audit_failed")
                .description(
                    "Integration audit events that failed to publish. Non-zero values indicate a gap in the audit "
                        + "trail.")
                .register(meterRegistry);
    }

    public void publish(IntegrationAuditEvent event, long integrationId, @Nullable Map<String, Object> data) {
        try {
            String principal;
            ...
            dataCopy.put("integrationId", integrationId);
            ...
            // committed by the time publishers run. Drift is observable via bytechef_integration_audit_failed.
            if (auditFailureCounter != null) {
                auditFailureCounter.increment();
            }

            log.error(
                "Failed to publish integration audit event {} (integrationId={}, data={})",
                event, integrationId, data, exception);
        }
    }
}
