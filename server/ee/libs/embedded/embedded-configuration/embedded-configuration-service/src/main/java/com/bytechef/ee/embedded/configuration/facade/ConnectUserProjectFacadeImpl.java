...
public class ConnectUserProjectFacadeImpl implements ConnectUserProjectFacade {
    ...
    @Override
    public void enableProjectWorkflow(Environment environment, String workflowReferenceCode, boolean enable) {
        ConnectedUserProject connectedUserProject = checkConnectedUserProject(
            OptionalUtils.get(SecurityUtils.getCurrentUserLogin(), "User not found"), environment);
    ...
    @Override
    public void publishProjectWorkflow(String workflowReferenceCode, Environment environment) {
        ConnectedUserProject connectedUserProject = checkConnectedUserProject(
            OptionalUtils.get(SecurityUtils.getCurrentUserLogin(), "User not found"), environment);
        ...
        int newProjectVersion = projectFacade.publishProject(connectedUserProject.getProjectId(), "", false);
        ...
        if (newProjectVersion == 2) {
            ...
            projectDeploymentFacade.updateProjectDeployment(
                projectDeploymentId, List.of(projectDeploymentWorkflow));
        }
    }
