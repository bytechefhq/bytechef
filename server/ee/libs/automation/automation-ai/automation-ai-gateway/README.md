...
> **Known limitation**: distributed-deploy support (a `*-remote-client` module that lets
> dataset/experiment endpoints land on a microservice that does not own the persistence
> layer) is not yet wired. The `@ConditionalOnEEVersion` + `bytechef.ai.gateway.enabled`
> gating across every module assumes a monolith deployment today.
