...
/**
 * Implementation of {@link AiEvalDatasetQueryFacade}. Carries the workspace-boundary guards that used to sit in
 * {@code AiEvalDatasetGraphQlController}'s method bodies, so they are enforced for every caller of the facade rather
 * than only the GraphQL entry point.
 *
 * <p>
 * These are role-RANK checks, not scope-token checks, and they stay that way — see
 * {@link AiEvalExperimentQueryFacadeImpl} for why neither half (the rank comparison, nor the null-workspace branch that
 * catches a dataset row with no workspace relation) survives translation into a {@code hasPermission} annotation.
 *
 * @version ee
 *
 * @author Ivica Cardic
...
class AiEvalDatasetQueryFacadeImpl implements AiEvalDatasetQueryFacade {

    /**
     * Mirrors the {@code MINIMUM_VIEWER_ROLE} constant on {@code AiEvalExperimentQueryFacadeImpl}. Duplicated as a
     * string literal because pulling in the EE typed enum here would cycle the GraphQL module on EE-only configuration.
     * Renaming {@code WorkspaceRole.VIEWER} will not break this at compile time — it is pinned by
     * {@code AiEvalDatasetQueryFacadeViewerRolePinTest} instead.
     */
    ...
    @Override
    public List<AiEvalDatasetView> getAiEvalDatasets(Long workspaceId) {
        if (workspaceId == null) {
            return List.of();
        }
    ...
    @Override
    public List<AiEvalDatasetVersionView> getAiEvalDatasetVersions(Long datasetId) {
        if (datasetId == null) {
            return List.of();
        }
    ...
    @Override
    public List<AiEvalDatasetItemView> getAiEvalDatasetItems(Long versionId) {
        if (versionId == null) {
            return List.of();
        }
        ...
        return aiEvalDatasetItemService.getItemsByVersion(version.getId())
            .stream()
            .map(AiEvalDatasetQueryFacadeImpl::toItemView)
            .toList();
    }
     ...
     * probing across workspaces (mirrors the pattern on {@code AiEvalExperimentComparisonFacade.experimentComparison}).
     */
