...
public class WorkspaceUser {
     ...
     *
     * @throws IllegalStateException when the stored ordinal names no environment. Spring Data JDBC fills the field
     *                               without the constructor's checks, and answering {@code null} would widen the row to
     *                               every environment.
     */
    public Environment getEnvironment() {
        ...
        if (environment < 0 || environment >= environments.length) {
            throw new IllegalStateException(
                "workspace_user id=" + id + " stores unknown environment ordinal " + environment);
        }
