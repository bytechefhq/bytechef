...
public interface CustomRoleService {
    ...
    /**
     * The built-in workspace roles and the scopes each one grants, least privileged first.
     *
     * <p>
     * Sits beside the scope catalogue rather than in a roles service of its own because it answers the same question
     * the catalogue does — what an operator is choosing between before composing a custom role. A role that merely
     * restates VIEWER is a role somebody has to maintain forever.
     */
    List<BuiltInRoleDTO> getBuiltInRoles();
    ...
    /**
     * Every permission scope a module contributed through {@code PermissionScopeProvider}, grouped by the module that
     * owns it.
     *
     * <p>
     ...
     * same place. The grouping comes from the same registration, so a module's scopes appear under its own heading
     * without the client having to infer a module from a scope name.
     */
    List<PermissionScopeGroupDTO> getPermissionScopeGroups();
