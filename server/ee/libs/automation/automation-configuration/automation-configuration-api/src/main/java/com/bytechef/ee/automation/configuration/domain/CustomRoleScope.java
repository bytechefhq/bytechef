 ...
 *
 * <p>
 * The scope is stored as its plain {@code String} name, and the set of valid names is assembled at runtime from the
 * per-module {@code PermissionScopeProvider} SPI rather than declared centrally. A name a module no longer contributes
 * therefore stays on disk and simply resolves to nothing, which is why every reader must fail closed on an unknown name
