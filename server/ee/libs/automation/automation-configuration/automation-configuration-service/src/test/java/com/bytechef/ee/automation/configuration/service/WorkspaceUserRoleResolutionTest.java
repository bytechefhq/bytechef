...
class WorkspaceUserRoleResolutionTest {
    ...
    @Test
    void testDeniesARowWhoseBuiltInRoleOrdinalIsOutOfRange() {
        // Spring Data JDBC hydrates workspace_role straight into the field, bypassing the constructor and setter range
        // checks, so a value the enum has no member for reaches the resolver. It must deny rather than raise
        // ArrayIndexOutOfBoundsException from inside a permission resolution, which no caller expects or handles.
        WorkspaceUser corruptedWorkspaceUser = mock(WorkspaceUser.class);

        when(corruptedWorkspaceUser.getCustomRoleId()).thenReturn(null);
        when(corruptedWorkspaceUser.getWorkspaceRole()).thenReturn(99);

        when(workspaceUserRepository.findByUserIdAndWorkspaceIdAndEnvironment(
            USER_ID, WORKSPACE_ID, Environment.PRODUCTION.ordinal())).thenReturn(Optional.of(corruptedWorkspaceUser));
