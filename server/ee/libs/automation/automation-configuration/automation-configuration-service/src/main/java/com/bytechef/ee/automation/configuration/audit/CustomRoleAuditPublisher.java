...
/**
 * Publishes custom-role audit events through Spring Boot's actuator audit bus. Mirrors {@code AiHubAuditPublisher} —
 ...
 *
 * <p>
 * Failures absorb silently into {@code bytechef_custom_role_audit_failed} + an error log; emission must never break the
...
public class CustomRoleAuditPublisher {

    private static final Logger log = LoggerFactory.getLogger(CustomRoleAuditPublisher.class);
    ...
    public CustomRoleAuditPublisher(
        ...
        this.auditFailureCounter = meterRegistry == null ? null
            : Counter.builder("bytechef_custom_role_audit_failed")
                .description(
                    "Custom-role audit events that failed to publish. Non-zero values indicate a gap in the audit "
                        + "trail.")
                .register(meterRegistry);
    }

    public void publish(CustomRoleAuditEvent event, @Nullable Map<String, Object> data) {
        try {
            String principal;
            ...
            // committed by the time publishers run. Drift is observable via bytechef_custom_role_audit_failed.
            if (auditFailureCounter != null) {
                auditFailureCounter.increment();
            }

            log.error(
                "Failed to publish custom-role audit event {} (data={})",
