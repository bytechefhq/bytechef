...
/**
 * Publishes project-user audit events through Spring Boot's actuator audit bus. Mirrors {@code AiHubAuditPublisher} —
 ...
 *
 * <p>
 * Failures absorb silently into {@code bytechef_project_user_audit_failed} + an error log; emission must never break
...
public class ProjectUserAuditPublisher {

    private static final Logger log = LoggerFactory.getLogger(ProjectUserAuditPublisher.class);
    ...
    public ProjectUserAuditPublisher(
        ...
        this.auditFailureCounter = meterRegistry == null ? null
            : Counter.builder("bytechef_project_user_audit_failed")
                .description(
                    "Project-user audit events that failed to publish. Non-zero values indicate a gap in the audit "
                        + "trail.")
                .register(meterRegistry);
    }

    public void publish(ProjectUserAuditEvent event, @Nullable Map<String, Object> data) {
        try {
            String principal;
            ...
            // committed by the time publishers run. Drift is observable via bytechef_project_user_audit_failed.
            if (auditFailureCounter != null) {
                auditFailureCounter.increment();
            }

            log.error(
                "Failed to publish project-user audit event {} (data={})",
