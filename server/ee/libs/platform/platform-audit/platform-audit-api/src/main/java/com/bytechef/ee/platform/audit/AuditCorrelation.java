...
/**
 * Thread-local carrier for a correlation ID that groups related audit events.
 *
 * <p>
 * The typical use case is a bulk facade method (e.g. {@code reassignAllConnections}) that triggers several nested
 * audited mutations (one {@code CONNECTION_REASSIGNED} per row). The parent establishes a correlation scope and every
 * child audit event picks up the same ID via {@link SpelAuditEngine}, so an auditor reviewing the stream can reassemble
 * the parent/child relationship without relying on temporal proximity.
 *
 * <p>
