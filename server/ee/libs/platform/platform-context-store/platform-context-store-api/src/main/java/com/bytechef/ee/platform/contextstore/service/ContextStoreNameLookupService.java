 ...
 * fallback rather than hardcoding the workspace-scoped store query here.
 *
 * <p>
 * The actual workspace-scoped name match is implemented automation-side (where the workspace scoping rules live) and
 * wired in via Spring DI. The platform-side action calls this SPI via {@code ObjectProvider} so deployments without the
 * automation-context-store module on the classpath start cleanly — the action's name-based path simply becomes
 * unreachable rather than blowing up at construction time.
 *
 * <p>
