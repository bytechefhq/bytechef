...
/**
 * The closed set of webhook event discriminators. Pins the wire names so a typo in a subscription row's {@code events}
 * JSON column fails at parse time rather than silently never firing. Dispatchers (e.g.
 * {@code AiObservabilityWebhookDeliveryServiceImpl}) compare their outgoing event name against this enum and reject
 * unknown values.
 *
 * <p>
 ...
 * {@code ["trace.completed","budget.exceeded"]}), NOT as INT ordinals. This deliberately diverges from the project
 * convention of persisting JDBC enums as INT ordinals (used by {@link AiGatewayProviderType} and similar scalar enum
 * columns). Reasons:
 * <ol>
...
public enum AiObservabilityWebhookEventType {
     ...
     * names bubble up as {@link IllegalArgumentException} so a typo cannot result in a silent "subscription that never
     * fires".
     */
