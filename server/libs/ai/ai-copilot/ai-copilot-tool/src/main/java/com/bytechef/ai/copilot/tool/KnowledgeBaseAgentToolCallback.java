...
/**
 * Hand-rolled Spring AI {@link ToolCallback} that exposes the Knowledge Base Copilot subagent to the parent ai_hub
 * agent.
 *
 * @author Ivica Cardic
 */
public class KnowledgeBaseAgentToolCallback implements ToolCallback {

    private static final Logger log = LoggerFactory.getLogger(KnowledgeBaseAgentToolCallback.class);
    ...
    public KnowledgeBaseAgentToolCallback(ChatClient knowledgeBaseChatClient) {
        this.knowledgeBaseChatClient = knowledgeBaseChatClient;
    }
    ...
    public String call(String toolInput, @Nullable ToolContext toolContext) {
        try {
            KnowledgeBaseAgentInput input = JsonUtils.read(toolInput, KnowledgeBaseAgentInput.class);
            ...
            String result = CurrentAgentContext.callWith(
                CopilotAgentType.KNOWLEDGE_BASE_AGENT, parentAgent,
                () -> knowledgeBaseChatClient.prompt(request)
                    .toolContext(forwardedContext)
                    .call()
                    .content());

            if (result == null) {
                log.warn("knowledge_base subagent returned null for request='{}'", request);

                return ToolErrors.toolError("knowledge_base subagent returned null");
            }

            return result;
        } catch (JacksonException exception) {
            log.warn(
                "knowledge_base_agent rejected malformed tool input: {} — first 200 chars of input: {}",
            ...
            return ToolErrors.runtimeFailure(
                KnowledgeBaseAgentToolCallback.class, "knowledge_base_agent", exception);
        }
    }
    ...
    public record KnowledgeBaseAgentInput(String request) {
    }
}
