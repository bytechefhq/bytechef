...
public class PropertyValidator {
    ...
    private static void validateArrayProperty(
        ...
        if (isUnionType) {
            ...
            FieldValidator.validateObjectArrayElementsWithWarnings(currentValue, arrayElementDef, propertyPath, errors,
                warnings);
        }
    }
        ...
        definitionNode.fieldNames()
            .forEachRemaining(propertyName -> {
                ...
                if (defValue.isTextual()) {
                    ...
                    validateNestedObject(currentNode, propertyName, defValue, propertyPath,
                        errors, warnings, originalTaskDefinition, originalTaskDefinitionForArrays,
    ...
    private static void validateArrayPropertyWithArraySupport(
        ...
        if (isUnionType) {
            ...
            if (defValue.size() == 1 && defValue.get(0)
                .isTextual() &&
                "task".equalsIgnoreCase(defValue.get(0)
                    .asText())) {
            ...
            if (arrayElementDef.isArray()) {
                ...
                for (int i = 0; i < currentValue.size(); i++) {
                    ...
                    if (!subArray.isArray()) {
                        ...
                        ValidationErrorBuilder.append(errors,
                            ValidationErrorBuilder.typeError(subArrayPath, "array", actualType));
                        continue;
                    }
                    ...
                    try {
                        ...
                        if (originalArrayDef != null && originalArrayDef.isArray() && originalArrayDef.size() > 0) {
                            ...
                            validateDiscriminatedUnionArray(subArray, originalSubArrayElementDef, subArrayPath, errors,
                                warnings);
                        } else {
                ...
                // Array of objects - use original task definition to get array element definition with display
                // conditions
                try {
                    JsonNode originalTaskDefNode = WorkflowParser.parseJsonString(originalTaskDefinitionForArrays);
                    JsonNode originalParametersNode = originalTaskDefNode.get("parameters");
                    ...
                    if (originalArrayDef != null && originalArrayDef.isArray() && originalArrayDef.size() > 0) {
                        ...
                        FieldValidator.validateObjectArrayElementsWithWarnings(currentValue, originalArrayElementDef,
                            propertyPath, errors, warnings);
                    } else {
                        // Fallback to processed definition
                        FieldValidator.validateObjectArrayElementsWithWarnings(currentValue, arrayElementDef,
                            propertyPath, errors, warnings);
                    }
                } catch (Exception e) {
                    // Fallback to processed definition
                    FieldValidator.validateObjectArrayElementsWithWarnings(currentValue, arrayElementDef, propertyPath,
                        errors, warnings);
                }
            }
        }
     ...
     */
    public static void
        validateTaskArray(JsonNode arrayValue, String propertyPath, StringBuilder errors, StringBuilder warnings) {
        for (int i = 0; i < arrayValue.size(); i++) {
            ...
            if (taskElement.has("parameters") && taskElement.has("type")) {
                ...
                if (!parameters.isObject()) {
                    ...
                    ValidationErrorBuilder.append(errors,
                        ValidationErrorBuilder.typeError(elementPath + ".parameters", "object", actualType));
                }
            }
        }
    }
    ...
    private static void validateDiscriminatedUnionArray(
        JsonNode arrayValue, JsonNode unionDef, String propertyPath, StringBuilder errors, StringBuilder warnings) {
        for (int i = 0; i < arrayValue.size(); i++) {
