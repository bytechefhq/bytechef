...
public class CustomIntegrationRepositoryImpl implements CustomIntegrationRepository {
    ...
    public List<Integration> findAllIntegrations(Long categoryId, List<Long> ids, Long tagId, Integer status) {
        ...
        for (Integration integration : integrations) {
            ...
            integration.setIntegrationWorkflows(
                jdbcClient
                    .sql(
                        "SELECT integration_workflow.workflow_id, integration_workflow.integration_version FROM integration_workflow WHERE integration_id = ?")
                    .param(integration.getId())
                    .query(IntegrationWorkflow.class)
                    .list()
                    .stream()
                    .map(
                        integrationWorkflow -> new com.bytechef.embedded.configuration.domain.IntegrationWorkflow(
                            integrationWorkflow.workflow_id, integrationWorkflow.integration_version))
