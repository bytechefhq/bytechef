    ...
    public Page<WorkflowExecution> getWorkflowExecutions(
        ...
        if (workflowId != null) {
            ...
            integration.fetchLastVersion()
                .ifPresent(lastVersion -> workflowIds.addAll(integration.getAllWorkflowIds()));
        } else {
            workflowIds.addAll(
                CollectionUtils.map(integrationFacade.getIntegrationWorkflows(), Workflow::getId));
        }
                    ...
                    CollectionUtils.getFirst(
                        ...
                        integration -> CollectionUtils.contains(
                            OptionalUtils.mapOrElse(
                                integration.fetchLastVersion(),
                                lastVersion -> integration.getAllWorkflowIds(), List.of()),
                            job.getWorkflowId())),
                    integrationInstanceConfigurationService.getIntegrationInstanceConfiguration(
                        integrationInstance.getIntegrationInstanceConfigurationId()),
                    integrationInstance,
