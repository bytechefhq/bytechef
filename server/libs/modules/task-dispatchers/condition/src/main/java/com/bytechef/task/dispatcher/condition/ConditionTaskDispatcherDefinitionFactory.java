...
public class ConditionTaskDispatcherDefinitionFactory implements TaskDispatcherDefinitionFactory {
    ...
    public ConditionTaskDispatcherDefinitionFactory(Optional<TaskListOutputDataSource> taskListOutputDataSource) {
        this.taskDispatcherDefinition = taskDispatcher(CONDITION)
            .title("Condition")
    ...
    private static ModifiableArrayProperty conditionsProperty() {
    ...
    protected static OutputResponse output(
        ...
        List<Map<String, ?>> caseTrueTasks = MapUtils.getList(
            inputParameters, CASE_TRUE, new TypeReference<>() {}, List.of());

        if (caseTrueTasks.isEmpty()) {
            return null;
        }

        Map<String, ?> lastTask = caseTrueTasks.getLast();
