 ...
 */
public class ElevenLabsStreamSpeechAction {

    public static final ModifiableActionDefinition ACTION_DEFINITION = action("streamSpeech")
        .title("Stream Speech")
        ...
        .output(
            outputSchema(
                fileEntry()
                    .description("Speech audio stream that was created.")))
        .perform(ElevenLabsStreamSpeechAction::perform);

    private ElevenLabsStreamSpeechAction() {
    }
    ...
    public static FileEntry perform(Parameters inputParameters, Parameters connectionParameters, Context context) {
        return context.http(http -> http.post(
            "/text-to-speech/" + inputParameters.getRequiredString(VOICE_ID) + "/stream"))
