...
Use **Create Run** for new work on Browser Use V4. It returns a run ID immediately. Use **Get Run** with that ID to read the final result, error, token count, and cost.

The existing **Create Session** and **Get Session** actions stay on V3 so current ByteChef workflows keep working.
