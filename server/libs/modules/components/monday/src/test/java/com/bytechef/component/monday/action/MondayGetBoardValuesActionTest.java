 ...
 */
class MondayGetBoardValuesActionTest {
    ...
    void testPerform() {
        ...
        try (MockedStatic<MondayUtils> mondayUtilsMockedStatic = mockStatic(MondayUtils.class)) {
            ...
            Object result = MondayGetBoardValuesAction.perform(parameters, parameters, mockedActionContext);

            assertEquals(Map.of(ID, "abc"), result);
            assertEquals(
                "query{boards(ids: 2){items_page{items{id name group{id title}column_values (ids: " +
                    "[\"3\",\"4\",\"5\"]){text value column{id title}}}}}}",
