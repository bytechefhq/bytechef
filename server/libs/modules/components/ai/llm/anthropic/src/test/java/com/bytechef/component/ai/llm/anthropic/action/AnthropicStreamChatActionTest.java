 ...
 */
class AnthropicStreamChatActionTest {
    ...
    void testOnSubscribeSetsTimeoutListenerAndRequestsAll() {
        ...
        verify(mockedEmitter).addTimeoutListener(timeoutCaptor.capture());

        timeoutCaptor.getValue()
            .run();

        verify(mockedSubscription).cancel();
        verify(mockedSubscription).request(Long.MAX_VALUE);
    }

    @Test
    void testOnNextSendsItemToEmitter() throws Exception {
        subscriber.onSubscribe(mockedSubscription);
        ...
        verify(mockedEmitter).send(item);
    }

    @Test
    void testOnNextLogsAndCancelsOnSendException() throws Exception {
        subscriber.onSubscribe(mockedSubscription);

        doThrow(new RuntimeException("send failed")).when(mockedEmitter)
            .send(any());

        subscriber.onNext("chunk");

        verify(mockedContext).log(any());
        verify(mockedSubscription).cancel();
    }
    ...
    void testOnErrorForwardsThrowableToEmitter() {
        ...
        verify(mockedEmitter).error(throwable);
    }
