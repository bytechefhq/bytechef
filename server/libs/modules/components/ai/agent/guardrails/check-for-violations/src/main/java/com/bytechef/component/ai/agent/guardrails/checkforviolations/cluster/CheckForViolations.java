...
public final class CheckForViolations {
    ...
    private ChatClient resolveSharedChatClient(
        ...
        if (!(model instanceof ChatModel chatModel)) {
            ...
            throw new IllegalArgumentException(
                "MODEL child '%s' on component '%s' v%s returned %s; CheckForViolations requires a ChatModel. Attach a chat-capable model."
                    .formatted(
                        element.getClusterElementName(),
                        element.getComponentName(),
                        element.getComponentVersion(),
                        returnedType));
        }
    ...
    private static boolean hasLlmChild(List<ResolvedCheckChild> resolvedChildren) {
        for (ResolvedCheckChild resolved : resolvedChildren) {
            GuardrailCheckFunction function = resolved.function();

            if (function.requiresChatClient()) {
                return true;
            }
        }
