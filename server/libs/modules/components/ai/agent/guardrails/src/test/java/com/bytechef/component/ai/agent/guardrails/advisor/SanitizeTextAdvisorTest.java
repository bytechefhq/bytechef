...
class SanitizeTextAdvisorTest {
    ...
    @Test
    void testGetOrderIsBeforeChatMemoryAdvisor() {
        SanitizeTextAdvisor advisor = SanitizeTextAdvisor.builder()
            .context(mock(Context.class))
            .build();

        assertThat(advisor.getOrder()).isEqualTo(Advisor.DEFAULT_CHAT_MEMORY_PRECEDENCE_ORDER - 1);
    }

    @Test
    void testSanitizedTextReachesChatMemoryAdvisor() {
        // Regression for two prior ordering bugs (commits 91b43c295c2, 652087e27d8): the simple value-equality test
        // above doesn't catch a chain-composition regression. Here we build a spy advisor that mimics
        // PromptChatMemoryAdvisor's order and capture what user text it sees — that's what would actually get persisted
        // to chat memory. If SanitizeTextAdvisor's order ever stops being strictly less than the chat-memory order,
        // unsanitized text would land in memory and this assertion fails.
        Parameters runOnInput = ParametersFactory.create(
            Map.of("validateInput", true, "validateOutput", true));
        GuardrailSanitizerFunction maskSensitive = (text, context) -> text.replace("my-secret", "<REDACTED>");

        SanitizeTextAdvisor sanitizeAdvisor = SanitizeTextAdvisor.builder()
            .add("mask", maskSensitive, runOnInput, ParametersFactory.create(Map.of()))
            .context(mock(Context.class))
            .build();

        AtomicReference<String> textObservedByChatMemoryAdvisor = new AtomicReference<>();

        CallAdvisor chatMemorySpy = new CallAdvisor() {
            @Override
            public ChatClientResponse adviseCall(ChatClientRequest request, CallAdvisorChain chain) {
                ...
                textObservedByChatMemoryAdvisor.set(userText);
            ...
            public String getName() {
                return "chatMemorySpy";
            }

            @Override
            public int getOrder() {
                return DEFAULT_CHAT_MEMORY_PRECEDENCE_ORDER;
            }
        };

        assertThat(sanitizeAdvisor.getOrder())
            .as("SanitizeTextAdvisor MUST run before chat-memory advisor, otherwise unsanitized text gets persisted")
            .isLessThan(chatMemorySpy.getOrder());

        // Compose a chain that delegates from sanitize → chatMemorySpy → terminal mock.
        CallAdvisorChain terminalChain = mock(CallAdvisorChain.class);

        when(terminalChain.nextCall(any(ChatClientRequest.class))).thenAnswer(invocation -> {
            ChatClientRequest forwarded = invocation.getArgument(0);

            return chatMemorySpy.adviseCall(forwarded, mock(CallAdvisorChain.class, invocation2 -> {
                ChatClientRequest finalRequest = invocation2.getArgument(0);
        ...
        sanitizeAdvisor.adviseCall(requestWithUser("please remember my-secret value"), terminalChain);

        assertThat(textObservedByChatMemoryAdvisor.get())
            .as("the chat-memory advisor must see the sanitized text, not the raw user input")
            .isEqualTo("please remember <REDACTED> value");
    }
    ...
    void testAdvisorIsThreadSafeAcrossConcurrentInvocations() {
        ...
        // sanitize() and must not leak between calls when the same advisor instance is reused by Spring AI's pool.
        // If the sanitizer saw state from another thread, the rewritten output would contain a substring the caller
        // never produced.
        ...
        IntStream.range(0, concurrency)
            ...
            .forEach(index -> {
                ...
                String rewritten = advisor.sanitizeForTesting(input);
