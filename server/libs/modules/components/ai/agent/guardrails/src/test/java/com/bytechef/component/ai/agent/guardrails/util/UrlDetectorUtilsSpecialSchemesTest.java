 ...
 * @author Ivica Cardic
 */
class UrlDetectorUtilsSpecialSchemesTest {
    ...
    void allowlistedMailtoSchemePasses() {
        ...
        List<UrlMatch> violations = UrlDetectorUtils.detectViolations("mailto:a@b.com", policy);
    ...
    void allowlistEntryMatchesWwwPrefixedHost() {
        ...
        List<UrlMatch> violations = UrlDetectorUtils.detectViolations("https://www.example.com/path", policy);
    ...
    void wwwPrefixedAllowlistEntryMatchesBareHost() {
        ...
        List<UrlMatch> violations = UrlDetectorUtils.detectViolations("https://example.com/path", policy);
    ...
    void detectsPunycodeHost() {
        ...
        List<UrlMatch> allowed = UrlDetectorUtils.detectViolations(
            "visit https://xn--e1afmkfd.xn--p1ai/page now", policyAllowingPunycode);

        assertThat(allowed).isEmpty();

        List<UrlMatch> blocked = UrlDetectorUtils.detectViolations(
    ...
    void hostCaseFoldingMatchesLowercaseAllowlist() {
        ...
        List<UrlMatch> violations = UrlDetectorUtils.detectViolations("go to https://EXAMPLE.COM/admin", policy);
