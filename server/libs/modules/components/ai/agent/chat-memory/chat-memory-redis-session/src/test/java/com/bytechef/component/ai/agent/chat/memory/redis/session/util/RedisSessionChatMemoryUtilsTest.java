...
class RedisSessionChatMemoryUtilsTest {
    ...
    @Test
    void testSameConnectionReusesOneJedisPool() {
        JedisPooled firstJedisPooled = RedisSessionChatMemoryUtils.getSharedJedisPooled(
            MockParametersFactory.create(Map.of("host", "localhost", "port", 6379)));
        JedisPooled secondJedisPooled = RedisSessionChatMemoryUtils.getSharedJedisPooled(
            MockParametersFactory.create(Map.of("host", "localhost", "port", 6379)));

        assertSame(firstJedisPooled, secondJedisPooled);
    }

    @Test
    void testDifferentConnectionsGetDifferentJedisPools() {
        JedisPooled firstJedisPooled = RedisSessionChatMemoryUtils.getSharedJedisPooled(
            MockParametersFactory.create(Map.of("host", "localhost", "port", 6379)));
        JedisPooled secondJedisPooled = RedisSessionChatMemoryUtils.getSharedJedisPooled(
            MockParametersFactory.create(Map.of("host", "localhost", "port", 6380)));

        assertNotSame(firstJedisPooled, secondJedisPooled);
    }
}
