 ...
 *
 * <p>
 * Each tenant gets its own S3 bucket: {@code {bucketPrefix}-{tenantId}}, or a sanitized form with a hash suffix when
 * that is not a valid S3 bucket name. The bucket is created on first use if it does not exist. Per-tenant
 * {@link S3SessionRepository} instances are cached in a Caffeine cache with a 1-hour expiry-after-access policy so idle
 * tenants don't pin memory indefinitely.
 *
 * @author Ivica Cardic
 */
public final class TenantRoutingS3SessionRepository implements SessionRepository {
    ...
    private SessionRepository createForTenant(String tenantId) {
        String bucketName = toBucketName(tenantId);
    ...
    private String toBucketName(String tenantId) {
