...
public class GithubSearchCodeAction {
    ...
    public static final ModifiableActionDefinition ACTION_DEFINITION = action("searchCode")
        ...
        .help("", "https://docs.bytechef.io/reference/components/github_v1#search-code")
        .properties(
            OWNER_PROPERTY,
            string(REPOSITORY)
                .label("Repository")
    ...
    private static String buildQuery(Parameters inputParameters) {
        ...
        query.append("%s repo:".formatted(inputParameters.getRequiredString(QUERY)));
        query.append("%s/%s".formatted(
    ...
    private static void appendIfPresent(StringBuilder query, String prefix, String value) {
        if (value != null) {
            query.append(" %s:%s".formatted(prefix, value));
        }
    }
}
