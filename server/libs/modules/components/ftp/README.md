...
`FtpUploadActionIntTest` reads FTP credentials and host from environment variables so the tests can run against an external server in CI or staging without hardcoding credentials.

| Environment Variable | Spring Property | Default | Description |
|---|---|---|---|
| `BYTECHEF_TEST_FTP_PASSWORD` | `bytechef.test.ftp.password` | `T3st@Pass123` | FTP/SFTP password |
| `BYTECHEF_TEST_FTP_USERNAME` | `bytechef.test.ftp.username` | `ftpuser` | FTP/SFTP username |
| `BYTECHEF_TEST_FTP_HOST_IP` | `bytechef.test.ftp.host.ip` | `127.0.0.1` | IP address embedded in the FTP PASV response |

**All three variables have defaults**, so no configuration is required for local development — the tests run against the Testcontainers-managed FTP container with the built-in credentials.

Override them when you need to run against a real FTP server or in a CI environment with different credentials:

```bash
export BYTECHEF_TEST_FTP_PASSWORD="your-password"
export BYTECHEF_TEST_FTP_USERNAME="your-username"
export BYTECHEF_TEST_FTP_HOST_IP="192.168.1.100"

./gradlew :server:libs:modules:components:ftp:testIntegration
```

**How Spring resolves env vars to properties:** Spring's `SystemEnvironmentPropertySource` automatically maps environment variable names to Spring property names by converting underscores to dots and uppercase to lowercase. `BYTECHEF_TEST_FTP_PASSWORD` becomes `bytechef.test.ftp.password`, which matches the `@Value("${bytechef.test.ftp.password:T3st@Pass123}")` annotation — no additional configuration needed.

`FtpDownloadFileActionIntTest` uses hardcoded `static final` constants and does not read any environment variables.
