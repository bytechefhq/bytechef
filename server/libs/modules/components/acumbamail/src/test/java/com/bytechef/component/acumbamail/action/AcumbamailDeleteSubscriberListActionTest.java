...
class AcumbamailDeleteSubscriberListActionTest extends AbstractAcumbamailActionTest {
    ...
    void testPerform() {
        Object result =
            AcumbamailDeleteSubscriberListAction.perform(mockedParameters, mockedParameters, mockedActionContext);

        assertEquals(responseMap, result);

        Object[] query = queryArgumentCaptor.getValue();
        assertEquals(List.of("auth_token", "test-token", LIST_ID, "test"), Arrays.asList(query));
    }
}
