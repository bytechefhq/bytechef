...
public class ScrapeGraphAiSearchScraperAction {
    public static final ComponentDsl.ModifiableActionDefinition ACTION_DEFINITION = action("searchScraper")
        ...
        .metadata(
            Map.of(
                "method", "POST",
                "path", "/searchscraper", "bodyContentType", BodyContentType.JSON, "mimeType", "application/json"
        ...
        .output(outputSchema(object()
            .properties(string("request_id").description("Unique identifier for the search request.")
                ...
                object("result").description("The search results.")
                    .required(false),
                array("reference_urls")
                    .items(string().description("List of URLs that were used as references for the answer."))
