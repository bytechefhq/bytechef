...
public class CustomActionUtils {
    ...
    protected static Object perform(Map<String, ?> inputParameters, ActionContext context) {
        return HttpClientUtils.exchange(
            ...
            MapUtils.getRequired(inputParameters, METHOD, HttpClientUtils.RequestMethod.class))
            .configuration(responseType(HttpClientUtils.ResponseType.JSON))
            .body(getBody(
                MapUtils.get(inputParameters, BODY_CONTENT_TYPE, BodyContentType.class), inputParameters))
            .headers(
                MapUtils.toMap(
                    MapUtils.getMap(inputParameters, HEADERS, Map.of()),
                    Map.Entry::getKey,
                    entry -> List.of((String) entry.getValue())))
            .queryParameters(
                MapUtils.toMap(
                    MapUtils.getMap(inputParameters, QUERY_PARAMETERS, Map.of()),
                    Map.Entry::getKey, entry -> List.of((String) entry.getValue())))
            .execute();
    }
