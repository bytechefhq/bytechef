...
/**
 * Verifies that the aggregator drops results from workspaces the caller cannot access and results without a
 * workspaceId.
 *
 * @author Ivica Cardic
 */
class AutomationSearchFacadeImplTest {
    ...
    @Test
    void testFiltersOutInaccessibleAndUnscopedWorkspaceResults() {
        ...
        SearchAssetProvider provider = new SearchAssetProvider() {
            ...
            public List<? extends SearchResult> search(String query, int limit) {
                return List.of(
                    ...
                    new TestResult(3L, null)); // no workspace
            }
