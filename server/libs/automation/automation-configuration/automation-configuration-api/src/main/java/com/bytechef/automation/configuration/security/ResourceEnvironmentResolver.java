...
public interface ResourceEnvironmentResolver {
    ...
    /**
     * Returns the environment the resource lives in, or empty when it has none — a deleted row, a resource not tied to
     * an environment, or an id of an unexpected shape. Empty falls back to the environment-unaware check.
     * <p>
     * An implementation must not catch its own failures and return empty: that would answer the check from the union of
     * every environment the caller can reach. Let the exception propagate, and the permission check denies.
     */
