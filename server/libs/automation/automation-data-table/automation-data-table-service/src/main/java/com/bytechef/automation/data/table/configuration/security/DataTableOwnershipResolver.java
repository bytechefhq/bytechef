...
package com.bytechef.automation.data.table.configuration.security;
...
/**
 * Maps a data table to its owning workspace through the {@code workspace_data_table} relation.
 *
 * <p>
 * Keyed on the data table's own id, not on the physical table's base name: the base name encodes an environment
 * ({@code dt_<env>_<name>}) and is not a stable identifier of the row, whereas the relation is. Fails closed when the
 * data table is assigned to no workspace.
 *
 * @author Ivica Cardic
 */
@Component
