...
class WorkspaceDataTableFacadeDuplicateTest {
    ...
    void testTheDuplicateJoinsTheSourceWorkspace() {
        ...
        when(workspaceDataTableService.getDataTableWorkspaceDataTables(DATA_TABLE_ID))
            .thenReturn(List.of(new WorkspaceDataTable(DATA_TABLE_ID, WORKSPACE_ID)));
    ...
    void testAnUnboundSourceLeavesTheDuplicateUnbound() {
        when(dataTableService.getBaseNameById(DATA_TABLE_ID)).thenReturn("table1");
        when(workspaceDataTableService.getDataTableWorkspaceDataTables(DATA_TABLE_ID)).thenReturn(List.of());
