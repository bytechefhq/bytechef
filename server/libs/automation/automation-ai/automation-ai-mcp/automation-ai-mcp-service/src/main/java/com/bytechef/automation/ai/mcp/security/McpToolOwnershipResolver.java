...
/**
 * Maps an MCP-tool id to its owning workspace by traversing tool &rarr; MCP component &rarr; MCP server &rarr;
 * {@code workspace_mcp_server} relation. Uses the platform repositories directly (not the {@code @PreAuthorize}-guarded
 * services) to avoid recursion. Fails closed when any hop cannot be resolved.
 *
 * @author Ivica Cardic
 */
@Component
public class McpToolOwnershipResolver implements ResourceOwnershipResolver {
    ...
    public McpToolOwnershipResolver(
        McpComponentRepository mcpComponentRepository, McpToolRepository mcpToolRepository,
