...
/**
 * Pins the {@code @PreAuthorize} expressions that close MCP-project IDOR (T20) at the facade tier. Per-project ops
 * resolve the owning workspace via {@code McpProject:ResourceRole}; create authorizes against the parent server via
 * {@code McpServer:ResourceRole}.
 *
 * @author Ivica Cardic
 */
class McpProjectFacadeAuthorizationTest {
    ...
    private static void assertExpression(String methodName, String expression) {
        ...
        for (Method candidate : McpProjectFacadeImpl.class.getDeclaredMethods()) {
