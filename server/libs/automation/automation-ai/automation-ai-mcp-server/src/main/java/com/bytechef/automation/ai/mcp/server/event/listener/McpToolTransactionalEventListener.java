...
package com.bytechef.automation.ai.mcp.server.event.listener;
...
public class McpToolTransactionalEventListener {
    ...
    public McpToolTransactionalEventListener(
        @Qualifier("automationMcpAsyncServer") McpAsyncServer automationMcpAsyncServer, ToolFacade toolFacade) {

        this.mcpAsyncServer = automationMcpAsyncServer;
        ...
        mcpAsyncServer.addTool(
            McpToolUtils.toAsyncToolSpecification(
                toolFacade.getFunctionToolCallback(
                    toolFacade.toTool(mcpToolSaveEvent.getEntity()))));
        mcpAsyncServer.notifyToolsListChanged();
    }

    @TransactionalEventListener
    public void handleEvent(AfterDeleteEvent<McpTool> mcpToolDeleteEvent) {
        ToolDTO toolDTO = toolFacade.toTool(mcpToolDeleteEvent.getEntity());
