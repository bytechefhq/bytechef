...
/**
 * Quartz job that fires on the cron schedule registered by {@code QuartzAgentScheduler}. Publishes an
 * {@link AgentScheduleFiredEvent} and returns — all real work (task creation, LLM dispatch) happens on the listener
 * thread so the Quartz worker pool is not pinned by long-running LLM calls.
 *
 * @author Ivica Cardic
 */
