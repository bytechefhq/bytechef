...
/**
 * Evaluates the real {@code @PreAuthorize} expressions on the job control endpoints through the real
 * {@link AutomationMethodSecurityExpressionHandler} and {@link AutomationPermissionEvaluator}, asserting each guard in
 * both directions and the exact check that reaches {@link PermissionService}. The guards sit on the controller rather
 * than on {@code JobFacade}, because the coordinator, the websocket and Twilio handlers, the deployment facade and the
 * synchronous executor stop and delete jobs through that facade with no user present.
 *
 * @author Ivica Cardic
 */
class JobApiControllerAuthorizationTest {
    ...
    private static Stream<GuardCase> guardCaseStream() {
        return Stream.of(
            new GuardCase(
                "getJob", Map.of("id", JOB_ID),
                permissionService -> permissionService.hasResourceScope(JOB_ID, "Job", "EXECUTION_VIEW")),
            new GuardCase("getJobsPage", Map.of("pageNumber", 0), PermissionService::isTenantAdmin),
            new GuardCase("getLatestJob", Map.of(), PermissionService::isTenantAdmin),
            new GuardCase(
                "restartJob", Map.of("id", JOB_ID),
                permissionService -> permissionService.hasResourceScope(JOB_ID, "Job", "DEPLOYMENT_EDIT")),
            new GuardCase(
                "stopJob", Map.of("id", JOB_ID),
                permissionService -> permissionService.hasResourceScope(JOB_ID, "Job", "DEPLOYMENT_EDIT")));
    }
    ...
    private static Method findMethod(String methodName) {
        List<Method> methods = Arrays.stream(JobApiController.class.getDeclaredMethods())
