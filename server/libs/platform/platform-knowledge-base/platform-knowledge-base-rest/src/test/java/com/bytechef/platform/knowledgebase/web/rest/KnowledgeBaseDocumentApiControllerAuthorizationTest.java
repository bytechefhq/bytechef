...
/**
 * Evaluates the real {@code @PreAuthorize} expression on {@link KnowledgeBaseDocumentApiController} through the real
 * {@link AutomationMethodSecurityExpressionHandler} and {@link AutomationPermissionEvaluator}, asserting the guard in
 * both directions and the exact check that reaches {@link PermissionService}.
 *
 * @author Ivica Cardic
 */
class KnowledgeBaseDocumentApiControllerAuthorizationTest {
    ...
    void testEveryGuardedMethodHasAnEvaluatedCase() {
        Set<String> guardedMethodNames = Arrays.stream(KnowledgeBaseDocumentApiController.class.getDeclaredMethods())
    ...
    private static Stream<GuardCase> guardCaseStream() {
        return Stream.of(
            new GuardCase(
                "uploadDocument", Map.of("id", KNOWLEDGE_BASE_ID),
                permissionService -> permissionService.hasResourceScope(
                    KNOWLEDGE_BASE_ID, "KnowledgeBase", "KNOWLEDGE_BASE_EDIT")));
    }
    ...
    private static Method findMethod(String methodName) {
        List<Method> methods = Arrays.stream(KnowledgeBaseDocumentApiController.class.getDeclaredMethods())
