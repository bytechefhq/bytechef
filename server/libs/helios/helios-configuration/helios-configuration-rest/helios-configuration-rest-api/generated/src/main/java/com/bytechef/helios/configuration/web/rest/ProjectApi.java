...
public interface ProjectApi {
    ...
    /**
     * POST /projects : Create a new project. Create a new project.
     *
    ...
    default ResponseEntity<ProjectModel> createProject(
        @Parameter(
            name = "ProjectModel", description = "", required = true) @Valid @RequestBody ProjectModel projectModel) {
    ...
    /**
     * POST /projects/{id}/workflows : Create new workflow and adds it to an existing project. Create new workflow and
    ...
    /**
     * DELETE /projects/{id} : Delete a project. Delete a project.
     *
     * @param id The id of a project. (required)
    ...
    /**
     * POST /projects/{id}/duplicate : Duplicates existing project. Duplicates existing project.
     *
     * @param id The id of a project. (required)
    ...
    /**
     * GET /projects/{id} : Get a project by id. Get a project by id.
     *
     * @param id The id of a project. (required)
     * @return The project object. (status code 200)
     */
        ...
        responses = {
            @ApiResponse(responseCode = "200", description = "A list of projects.", content = {
                @Content(
                    ...
                    array = @ArraySchema(schema = @Schema(implementation = ProjectModel.class)))
            })
        })
    @RequestMapping(
        method = RequestMethod.GET,
    ...
    default ResponseEntity<List<ProjectModel>> getProjects(
        @Parameter(
            name = "categoryId", description = "The category id used for filtering projects.",
            in = ParameterIn.QUERY) @Valid @RequestParam(value = "categoryId", required = false) Long categoryId,
        @Parameter(
            name = "projectInstances", description = "Use for filtering projects for which project instances exist.",
            in = ParameterIn.QUERY) @Valid @RequestParam(
                value = "projectInstances", required = false) Boolean projectInstances,
        @Parameter(
            name = "tagId", description = "The tag id of used for filtering projects.",
            in = ParameterIn.QUERY) @Valid @RequestParam(value = "tagId", required = false) Long tagId,
        @Parameter(
            name = "published", description = "Use for filtering published projects.",
            in = ParameterIn.QUERY) @Valid @RequestParam(value = "published", required = false) Boolean published) {
        getRequest().ifPresent(request -> {
            for (MediaType mediaType : MediaType.parseMediaTypes(request.getHeader("Accept"))) {
                if (mediaType.isCompatibleWith(MediaType.valueOf("application/json"))) {
                    String exampleString =
    ...
    /**
     * PUT /projects/{id} : Update an existing project. Update an existing project.
     *
     * @param id           The id of a project. (required)
     * @param projectModel (required)
     * @return The updated project object. (status code 200)
